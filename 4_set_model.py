...
gsearch4 = GridSearchCV(estimator = GB(learning_rate=0.1, n_estimators=gsearch1.best_params_['n_estimators'],max_depth=gsearch2.best_params_['max_depth'], min_samples_leaf =gsearch3.best_params_['min_samples_leaf'], min_samples_split =gsearch3.best_params_['min_samples_split'], random_state=0), param_grid = param_test4, scoring='recall',iid=False, cv=cv)
gsearch4.fit(train,train_y)
gsearch4.grid_scores_, gsearch4.best_params_, gsearch4.best_score_

# 调节参数subsample
param_test5 = {'subsample':[0.6,0.7,0.75,0.8,0.85,0.9]}
gsearch5 = GridSearchCV(estimator = GB(learning_rate=0.1, n_estimators=gsearch1.best_params_['n_estimators'],max_depth=gsearch2.best_params_['max_depth'], min_samples_leaf =gsearch3.best_params_['min_samples_leaf'], min_samples_split =gsearch3.best_params_['min_samples_split'], max_features=gsearch4.best_params_['max_features'],random_state=0), param_grid = param_test5, scoring='recall',iid=False, cv=cv)
gsearch4.fit(train,train_y)
gsearch5.grid_scores_, gsearch5.best_params_, gsearch5.best_score_

# 调参后的GDBT,这时可以通过减半步长，n_estimators加倍等调整来增加模型的泛化能力
model=GB(learning_rate=0.1/2, n_estimators=gsearch1.best_params_['n_estimators']*2,max_depth=gsearch2.best_params_['max_depth'], min_samples_leaf =gsearch3.best_params_['min_samples_leaf'],min_samples_split =gsearch3.best_params_['min_samples_split'], max_features=gsearch4.best_params_['max_features'],subsample=gsearch5.best_params_['subsample'],random_state=0)
model.fit(train,train_y)
pred=model.predict(test)
metrics.recall_score(test_y,pred)
...
model=GB(learning_rate=0.1, n_estimators=78,max_depth=5, min_samples_leaf =1,min_samples_split =52,random_state=0)
